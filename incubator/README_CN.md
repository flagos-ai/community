...
3. **TSC 条件批准**：捐赠方在 TSC 例会上答辩（约 30 分钟），TSC 按第 4 节规则投票。通过则合并提案 PR。
...
5. **正式接收**：SGA 生效且 IP 清理清单全部完成后，由 TSC（或其授权负责人）在提案文档中记录 **Final Acceptance**（含日期与核验依据），**正式接收自该记录之日生效**。仓库迁入 `flagos-ai` org（保留 fork 关系与 star）、README 标注 `(incubating)`、更新本页项目列表、发布公告，均为正式接收后的执行动作，按[正式接收执行手册](acceptance-runbook.md)完成。

## 6. 孵化、年度 review 与转正

### 孵化期

- 每个项目由 TSC 指派 **1~2 名 Mentor**，负责辅导治理落地、解答流程问题、在转正时出具推荐意见。Mentor 的产生、职责与更换见 [Mentor 指南](mentor-guide.md)。
...
转正由项目 maintainer 或 Mentor 按[转正提案模板](graduation-template.md)发起（逐维度附证据 + Mentor 书面推荐），TSC 按第 4 节规则投票通过后生效，去掉 `(incubating)` 标识。

> 各维度的量化参考指标（机构数量、发布次数等）由 TSC 后续补充，补充仅细化参考线，不改变上述维度本身。

## 7. 退出与归档

归档是**已正式接收的项目**（孵化中或正式项目）不再满足相应条件时的中性退出机制；未完成正式接收的提案按第 5 节的条件批准有效期处理，不进入归档。**发现机制**：孵化项目经年度 review 发现；正式项目**每 2 年**做一次轻量健康检查（复用[年度 review 模板](annual-review-template.md)），任何社区成员亦可随时依据下列情形发起提议。出现以下任一情形，由 TSC 按第 4 节规则投票决定：
...
归档后仓库设为**只读并保留**，具体执行（通知期、渠道处置、商标、复活机制）见[归档执行手册](archiving-runbook.md)。**对捐赠方的承诺**：SGA 授予的代码许可**不可撤销**，归档不影响任何人继续 fork 和使用；项目商标可与捐赠方协商返还。
...
CI 中的许可证扫描会在 PR 级别阻断违规依赖。完整分类（含个案裁定类）、CI 扫描要求及模型权重/数据集的许可规则见[许可证政策细则](license-policy.md)。

### 安全漏洞响应

孵化及正式项目的安全漏洞请**私密报告**至 <security@flagos.io>，请勿公开提 issue。我们会在 3 个工作日内确认，并与报告者协调披露时间线。完整流程（响应时限、披露协调、项目义务）见[安全响应政策](security-policy.md)。
...
| [license-policy.md](license-policy.md) | 许可证政策细则（分类、CI 扫描、AI 制品许可） |
| [security-policy.md](security-policy.md) | 安全响应政策（时限、流程、项目义务） |
| [mentor-guide.md](mentor-guide.md) | Mentor 指南（产生、职责、更换） |
| [acceptance-runbook.md](acceptance-runbook.md) | 正式接收执行手册（迁移与发布操作） |
...
| [archiving-runbook.md](archiving-runbook.md) | 归档执行手册（通知期、处置、复活） |

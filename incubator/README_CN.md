...
5. **正式接收**：仓库迁入 `flagos-ai` org（保留 fork 关系与 star），README 顶部标注 `(incubating)`，更新本页项目列表，发布公告。**正式接收自仓库迁入时生效。**
...
归档是项目**不再满足持续孵化或正式项目条件时的中性退出机制**。出现以下任一情形，任何社区成员可发起归档提议，由 TSC 按第 4 节规则投票决定：

- 连续 12 个月无实质活动（无代码提交、无发布、maintainer 失联）；
- 长期无法完成 IP 清理或修复合规问题；
- 失去维护团队且无人接手，或长期无 Mentor 可指派；
- 持续违反社区治理或行为准则且无改善；
- 无法维持基本的发布与安全响应能力；
- 项目维护团队主动申请退出。

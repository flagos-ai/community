...
TSC 依据以下原则评审捐赠提案，接收与否的理由公开记录。原则采用编号，便于孵化器其他文档（如提案模板）稳定引用。候选项目至少应：

1. 符合 FlagOS 的使命与技术范围（多芯片 AI 系统软件栈及其生态）；
2. 采用符合本文件第 9 节许可证政策的开源许可证；
3. IP 状况原则上可清理（存在已知问题不必然否决，但须有可行的清理路径）；
4. 有明确的初始维护团队，而非停止维护后的代码托管；
5. 愿意接受开放、中立的社区治理，包括本社区的 [GOVERNANCE_CN.md](../GOVERNANCE_CN.md) 与 [CODE_OF_CONDUCT_CN.md](../CODE_OF_CONDUCT_CN.md)；
6. 与 FlagOS 现有项目定位互补或有清晰的差异化，不造成社区内的重复竞争。

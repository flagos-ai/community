...
TSC 依据以下原则评审捐赠提案，接收与否的理由公开记录。每条原则带有稳定标号（**AP-n**），供孵化器其他文档交叉引用；标号一经分配不再变更、不复用——新增原则无论插入列表何处，均取下一个未用标号。候选项目至少应：

1. **AP-1（使命契合）**——符合 FlagOS 的使命与技术范围（多芯片 AI 系统软件栈及其生态）；
2. **AP-2（许可证）**——采用符合本文件第 9 节许可证政策的开源许可证；
3. **AP-3（IP 可清理）**——IP 状况原则上可清理（存在已知问题不必然否决，但须有可行的清理路径）；
4. **AP-4（维护承诺）**——有明确的初始维护团队，而非停止维护后的代码托管；
5. **AP-5（中立治理）**——愿意接受开放、中立的社区治理，包括本社区的 [GOVERNANCE_CN.md](../GOVERNANCE_CN.md) 与 [CODE_OF_CONDUCT_CN.md](../CODE_OF_CONDUCT_CN.md)；
6. **AP-6（互补性）**——与 FlagOS 现有项目定位互补或有清晰的差异化，不造成社区内的重复竞争。
...
6. **去单点依赖**：对原始捐赠方的单点依赖已明显降低（maintainer 与关键项目知识不集中于单一个人或机构）。

...
转正由 Mentor 推荐、TSC 按第 4 节规则投票通过后生效，去掉 `(incubating)` 标识。

> 各维度的量化参考指标（机构数量、发布次数等）由 TSC 后续补充，补充仅细化参考线，不改变上述维度本身。

## 7. 退出与归档

归档是**已正式接收的项目**（孵化中或正式项目）不再满足相应条件时的中性退出机制；未完成正式接收的提案按第 5 节的条件批准有效期处理，不进入归档。出现以下任一情形，任何社区成员可发起归档提议，由 TSC 按第 4 节规则投票决定：
...
归档后仓库设为**只读并保留**。**对捐赠方的承诺**：SGA 授予的代码许可**不可撤销**，归档不影响任何人继续 fork 和使用；项目商标可与捐赠方协商返还。

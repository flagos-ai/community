...
> Operational runbook for step ⑤ of the project contribution process in Section 6 of the [README](README.md): after Final Acceptance is recorded, the execution team completes asset migration and announcements per this document.
...
1. The contributing party hands over repository owner rights or uses GitHub **Transfer ownership** to move the repository into the `flagos-ai` org (stars, forks, issue and PR history are preserved; GitHub auto-redirects the old URL).
...
- PyPI / npm / Docker Hub etc.: add the alliance service account as owner; retain, downgrade, or remove existing organization and individual accounts according to the approved migration plan;
...
- Update the project list in the incubator [README](README.md) / [README_CN.md](README_CN.md) (status Incubating, contributing party, Mentors, proposal link);
...
- Publish the acceptance announcement in the community repo's GitHub Discussions (project intro, contributing party, incubating-status note);

...
- [ ] SGA 已签署生效，签署确认记录归档至 `projects/<项目名>/`（法律文本本体不入公开仓库，存联盟档案）
...
- 添加 `SECURITY.md`（指向[安全响应政策](security-policy_CN.md)）、确认 `LICENSE`/`NOTICE` 符合[许可证政策](license-policy_CN.md)；
- **项目级安全就绪**（[安全响应政策](security-policy_CN.md)第 6 节，**对外公告或开放贡献前（以先到者为准）**完成）：启用 GitHub Private Vulnerability Reporting、登记项目安全联系人至 `projects/<项目名>/` 档案。

## 3. 发布渠道与账号

按 IP 清理清单"资产核验与迁移准备"节确认的资产清单与迁移方案，逐项执行实际移交：

- PyPI / npm / Docker Hub 等：将联盟服务账号添加为 owner；根据已批准的迁移方案，保留、降权或移除现有的机构账号和个人账号；
- 域名：完成转移或 DNS 控制权移交；

...
- **vllm_flaggems**: Whitelist control file (`/root/flaggems_ops_control.json`) with `{"include": [enabled_ops]}`
...
```
L0 (FlagTree, FlagCX)
 └── L1 (FlagGems — compiled by FlagTree)
      └── L2 (vllm-plugin-FL — uses FlagGems operators + FlagCX communication)
           └── L3 (FlagRelease — orchestrates all above)
```

FlagRelease does not modify any L0-L2 component. It consumes their interfaces and reports issues back when failures are detected.

### Failure Handling

The pipeline never terminates on failure (except Claude API unavailability). Instead:
1. Diagnose the failure (log analysis, operator isolation)
2. File a GitHub issue against the responsible component
3. Mark `workflow.xxx_ok = false`
4. Continue to the next step
5. Publish as private (unqualified) at step 8

### Recovery

On session interruption, the next session reads `context.yaml` and `failure_diagnosis.json` to resume from the exact interruption point. `run_pipeline.sh` also provides post-exit fallback: syncing data, re-running publish if missed, and generating missing reports.

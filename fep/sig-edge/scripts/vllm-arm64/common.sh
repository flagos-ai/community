...
export WORK_DIR="$(realpath -m -- "${WORK_DIR:-$HOME/arm64-vllm024-test}")"
export MODEL_ROOT="$(realpath -m -- "${MODEL_ROOT:-$HOME/Models}")"
export W4_SOURCE="$(realpath -m -- "${W4_SOURCE:-$MODEL_ROOT/MiniCPM5-2B-W4A8-arm-FlagOS}")"
export W4_PACKED="$(realpath -m -- "${W4_PACKED:-$MODEL_ROOT/MiniCPM5-2B-W4A8-arm-FlagOS-packed}")"
export W8_MODEL="$(realpath -m -- "${W8_MODEL:-$MODEL_ROOT/MiniCPM5-2B-W8A8-arm-FlagOS}")"
...
export TRITON_CACHE_DIR="$(realpath -m -- "${TRITON_CACHE_DIR:-$WORK_DIR/.cache/triton-2c35990}")"
export TORCHINDUCTOR_CACHE_DIR="$(realpath -m -- "${TORCHINDUCTOR_CACHE_DIR:-$WORK_DIR/.cache/inductor-2c35990}")"
export VLLM_CACHE_ROOT="$(realpath -m -- "${VLLM_CACHE_ROOT:-$WORK_DIR/.cache/vllm-2c35990}")"

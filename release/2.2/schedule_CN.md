...
| 日期 | 节点 | 含义 |
|------|------|------|
| **2026-08-31** | **特性冻结(Feature Freeze)** | 目标 2.2 的 FEP 须已合入,且实现代码已提交至各模块仓库。此日期后:只做验收测试、bug 修复、调优和 FEP 文档更新——不再有新 FEP 进入 2.2,不再进 2.2 特性代码。后续版本的特性开发在 `main` 上继续。 |
...
- **进入即算数**:FEP PR 带 `target/2.2` 标签合入即进入 2.2([FEP 生命周期](../../fep/README_CN.md#fep-生命周期))。特性冻结同时关上 2.2 的两扇门——FEP 合入与 2.2 特性代码。
